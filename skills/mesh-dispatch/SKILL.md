---
name: mesh-dispatch
description: From the master session, queue a command for a worker session to execute autonomously on its next poll. Use when the user wants to tell another session (by label, session id, or all) to run something. Target a label like docs-rewrite or PMP-211, a session id, or * for broadcast.
allowed-tools:
  - Bash
---
...
1. Parse from the user request: `target` (a worker label e.g. `docs-rewrite` or `PMP-211`, a session_id, or `*`), `kind` (`shell` = run a shell command in the worker's worktree; `prompt` = have the worker carry out a task; `control` = `stop`), and `payload` (the command/prompt text).
